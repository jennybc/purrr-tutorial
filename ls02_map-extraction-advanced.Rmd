...
The [repurrrsive package](https://cran.r-project.org/package=repurrrsive) provides information on 6 GitHub users in an R list named `gh_users`.
...
1. Use `map_dfr()` to create a data frame with one row per user and variables for "name", "following", and "created_at". What type are the variables?
...
The [repurrrsive package](https://cran.r-project.org/package=repurrrsive) provides this in an R list named `gh_repos`.
