...
Atomic vectors are homogeneous. Each "atom" has the same flavor, by which I roughly mean type or storage mode, and is a scalar, by which I mean "has length one". The above examples show the most prevalent flavors of R vectors, though you will eventually encounter more exotic ones.

Exercises

1. Define the vectors above or similar. Use the family of `is.*()` functions to confirm vector type, e.g. `is.logical()`. You might need to guess or look some of them up.
...
To index a vector is to address specific elements or atoms, either for reading or writing. We index a vector using square brackets, like so: `x[something]`. There are several ways to express which elements you want, i.e. there are several valid forms for `something`:
...
I end the discussion of atomic vectors with two specific examples of "being intentional about type".
  ...
  * Use `L` to explicitly request integer. This looks weird but is a nod to short versus long integers. Just accept that this method of requesting a "long" is how we force a number to be an integer in R.
...
A list is actually still a vector in R, but it's not an atomic vector. We construct a list explicit with `list()` but, like atomic vectors, most lists are created some other way in real life.

```{r}
(x <- list(one = 1:3, two = 4:6))
(y <- list(logical = TRUE, integer = 4L, double = 4 * 1.2, character = "character"))
```

Exercises

1. Make the lists `x` and `y` shown above. Use `is.*()` functions to get to know these objects. Try to get some positive and negative results, i.e. establish a few things that `x` is and is NOT. Make sure to try `is.vector()` and `is.atomic()`.
...
Lots of operations are naturally vectorized in R. But almost always for atomic vectors. Not for lists.
...
Note the whole point of this tutorial is to show higher-level abstractions for this. You won't be writing these for loops yourself!
